...
export const scheduleJobs = () => {
  const cronExpression = "*/50 * * * *"; // Every 50 minutes
  ...
  try {
        ...
        try {
          ...
          for (const conversation of conversationsToProcess) {
            try {
              ...
              if (!abandonedConversation) {
                console.error(
                  `Failed to abandon conversation ${conversation.id}`
                );
                continue;
              }
              ...
              console.log(
                `Summary generated for conversation ${conversation.id}`
              );
          ...
          for (const formId of formIds) {
            try {
              ...
              console.error(
                `Error generating summary for form ${formId}:`,
                error
              );
            }
          }
    ...
    console.log(`Scheduled cron job with expression: ${cronExpression}`);
  } catch (error) {
    console.error(
      `Error scheduling cron job with expression ${cronExpression}:`,
      error
    );
  }
};
